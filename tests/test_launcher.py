...
def get_apptainer_or_singularity():
    if subprocess.run(['which apptainer'], shell=True, capture_output=True).stdout is not None:
        apptainer_or_singularity = 'apptainer'
    elif subprocess.run(['which singularity'], shell=True, capture_output=True).stdout is not None:
