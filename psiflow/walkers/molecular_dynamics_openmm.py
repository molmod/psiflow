...
def main():
    ...
    if args.force_threshold is not None:
        print(
            "IGNORING requested force threshold at {} eV/A!".format(
    ...
    if args.model_cls == "MACEModel":
        ...
        system = potential.createSystem(
            topology, device=args.device
        )  # periodic inferred from system
    system.addForce(openmm.CMMotionRemover())
    if args.temperature is not None:
        temperature = args.temperature * unit.kelvin
        integrator = openmm.LangevinMiddleIntegrator(
            temperature, 1.0 / unit.picoseconds, args.timestep * unit.femtosecond
        )
        ...
        velocities = get_velocities_at_temperature(
            300, atoms.get_masses()
        )  # NVE at 300 K
    if args.device == "cuda":
        platform_name = "CUDA"
    else:
    ...
    print(
        "check whether all interatomic distances > {}".format(args.distance_threshold)
    )
    state = trajectory[-1]
