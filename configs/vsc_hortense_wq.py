...
class SlurmProvider(ClusterProvider, RepresentationMixin):
    ...
    def submit(self, command, tasks_per_node, job_name="parsl.slurm"):
        ...
        retcode, stdout, stderr = self.execute_wait("sbatch {1} {0}".format(channel_script_path, '--partition={}'.format(self.partition)))
...
def get_config(path_internal):
    ...
    training = WorkQueueExecutor(
            ...
            port=9123,
            max_retries=0,
            init_command='export OMP_NUM_THREADS={}'.format(cores_per_gpu),
            worker_options='--gpus=1 --cores=12 --wall-time={}'.format(60 * 60), # set walltime
            )
    ...
    reference = WorkQueueExecutor(
            ...
            port=9124,
            max_retries=0,
            init_command='export OMP_NUM_THREADS=1', # necessary to set again
            worker_options='--gpus=0 --cores=32 --wall-time={}'.format(55 * 60), # set walltime
            )
